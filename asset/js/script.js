...
//=================================================== FORM CONTACT GOOGLE SPREDSHEET=========================================================
const scriptURL = "https://script.google.com/macros/s/AKfycbz88y41bYEqZ4n96nbkXfBTAHf_vMLbpl9ha_bHYWCHBC4s2Xyy_Mq-Xeh_GuVyYkNcgg/exec";
const form = document.forms["RezaMahendra-Contact-Form"];
const tombolKirim = document.querySelector(".tombol-kirim");
const animasiLoading = document.querySelector(".animasi-loading");
const myAlertSuccess = document.querySelector(".myalert-success");
const myAlertError = document.querySelector(".myalert-error");

// Fungsi untuk menampilkan pop-up success
function showSuccessPopup() {
  myAlertSuccess.classList.remove("d-none");

  // Fungsi untuk menghilangkan pop-up success ketika di klik tombol ok
  function hidePopupSuccess() {
    myAlertSuccess.classList.add("d-none");
  }

  // Menambahkan event listener pada tombol OK pada pop-up success
  const tombolPopUpSuccess = document.querySelector(".tombol-popup-success");
  tombolPopUpSuccess.addEventListener("click", hidePopupSuccess);
}
...
function showErrorPopup() {
  ...
  // Fungsi untuk menghilangkan pop-up error
  function hidePopupError() {
    myAlertError.classList.add("d-none");
  }

  // Menambahkan event listener pada tombol OK pada pop-up error
  const tombolPopUpError = document.querySelector(".tombol-popup-error");
  tombolPopUpError.addEventListener("click", hidePopupError);
}

form.addEventListener("submit", (e) => {
  e.preventDefault();

  // Tampilkan animasi loading saat tombol kirim diklik, dan sembunyikan tombol kirim saat form dikirimkan
  animasiLoading.classList.toggle("d-none");
  tombolKirim.classList.toggle("d-none");

  fetch(scriptURL, { method: "POST", body: new FormData(form) })
    .then((response) => {
      console.log("Success!", response);
      // setelah succes terkirim form akan di reset atau di kosongkan kembali
      form.reset();

      // Setelah pesan terkirim dan succes akan Tampilkan tombol kirim kembali,
      animasiLoading.classList.toggle("d-none");
      //lalu sembunyikan animasi loading
      tombolKirim.classList.toggle("d-none");

      // setelah succes pesan terkirim , Tampilkan pop-up success dan jeda 1 detik
    ...
    .catch((error) => {
      ...
      // Sembunyikan animasi loading dan tampilkan tombol kirim setelah jeda 1 detik
      setTimeout(() => {
        animasiLoading.classList.toggle("d-none");
        tombolKirim.classList.toggle("d-none");

        // Tampilkan pop-up error setelah jeda 1 detik
        setTimeout(showErrorPopup, 800);
      }, 1000);
    });
});
//============================================================================================================//
...
function openNewWindow(event) {
  event.preventDefault(); // Mencegah perilaku standar dari tautan
